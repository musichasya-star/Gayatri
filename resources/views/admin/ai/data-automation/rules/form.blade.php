...
@section('content')
    ...
    <div class="card"><div class="card-body"><form method="POST" action="{{ $rule->exists ? route('admin.ai.data-automation.rules.update', $rule) : route('admin.ai.data-automation.rules.store') }}" class="grid grid-2">@csrf @if($rule->exists) @method('PUT') @endif
        ...
        <label><span class="muted" style="display:block;margin-bottom:.4rem;font-weight:700;">Target Entity</span><select class="input" name="target_entity" required>@foreach(['customer','booking','followup'] as $target)<option value="{{ $target }}" @selected(old('target_entity', $rule->target_entity)===$target)>{{ $target }}</option>@endforeach</select></label>
